...
def main():
    ...
    if not os.path.isfile(model_path):
        raise ValueError(f'Model {args.model_name} does2 not exist.')
    ...
    #face_enhancer.face_helper.upscale_factor=args.outscale
    ...
    for idx, path in enumerate(paths):
        ...
        try:
            if args.face_enhance:
                print("face_enhance_x4plus", idx, int(time.mktime(time.localtime(time.time()))))
                _, _, output = face_enhancer.enhance(img, has_aligned=False, only_center_face=False, paste_back=True)
                print("face_enhance_x4plus", idx, int(time.mktime(time.localtime(time.time()))))

            else:
                print("x4plus", idx, int(time.mktime(time.localtime(time.time()))))
